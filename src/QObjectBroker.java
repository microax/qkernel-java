...
public final class QObjectBroker extends Joa 
{
    ...
    // RETURN:  JSONObject
    //--------------------------------------------------------------------------------
    public JSONObject jsonObjectRequest(QMessage q, com.qkernel.http.HTTPServer.Request req)
    {
        Method method = null;
	...
	try
	{
            method = objectClass.getMethod(m, new Class[] {req.getClass()} );
	    reply = (JSONObject)method.invoke(objHandler, new Object[] {req} );
	}
	catch(Exception e)
	{
