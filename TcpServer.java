package com.qkernel;
//
// TcpServer.java        TCP/IP Server class.
// ----------------------------------------------------------------------------
// History:
// --------
...
public final class TcpServer extends Eos
{
	...
	//----------------------------------------
    	...
    	//----------------------------------------
    	user_agent.socket = (Socket)accept_list.Dequeue();
    	//----------------------------------------
	...
	Socket sock;
	    ...
	    if(!user.socket.isClosed())
                user.socket.close();	// Try to close socket...
        }
        catch(Exception e)
	{
	...
	if((sock = (Socket)accept_list.Dequeue()) != null)
	{
	    //----------------------------------------
    	    // Setup UA handle's socket.
    	    //----------------------------------------
    ...
    public TcpServer(String n, Daemon d)
    {
        super(n, d);
    }
}
